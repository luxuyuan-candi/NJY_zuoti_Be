...
- 支持首页教学视频和学习推广内容管理。
- `doc/` 目录下的电子教材作为公开 PDF 上传到 MinIO `public-assets/docs/ebooks/`，由首页“学习推广”直接引用。
- 首页顶部教学视频拆分为多个短视频专题片段，统一存放在 MinIO `public-assets/video/` 与 `public-assets/images/`。
- 支持题目离线缓存和缓存版本更新。
...
| `docs/ebooks/*.pdf` | 首页学习推广电子教材 PDF |
...
- 首页教学视频配置。
- 学习推广内容、Banner、公告配置。
- 当前首页“学习推广”用于陈列电子教材，点击后直接在线打开 MinIO 公开 PDF。
- 推广内容跳转目标配置。
- 图片、视频上传和资源引用管理。
- 当前首页公告配置由 `home_contents` 表持久化，供首页滚动公告和公告详情页共用。
...
- 错题多次答对自动移出。
- 离线作答数据同步。
- 对 `GUEST` 角色拒绝练习、记录、错题、收藏与趋势统计访问。
...
- 记录页当前实现
  ...
  - “总做题数”“正确率”“考试数”“错题本”“趋势统计”均由后端持久化记录聚合
  - 正确率按总作答次数计算，错题按错误发生次数累计，不按题目去重
  - 当前“考试数”展示口径按完成次数统计，而不是独立考试套卷数量
  - “查看错题”读取单次记录下的错题明细；“错题本”读取 `mistakes` 聚合表并展示 `wrong_times`
